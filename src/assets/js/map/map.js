export default class HDMap {
  ...
  initMap (mapDiv, params) {
    ...
    this.map = new ol.Map({
      ...
      view: new ol.View({
        ...
        projection: that.projection,
        extent: that.fullExtent,
        resolution: that._resolutions[params['resolution']],
  ...
  addArcGisLayer () {
    let templayer = new ol.layer.Image({
      ...
      source: new ol.source.ImageArcGISRest({
        url: 'http://211.101.37.251:6080/arcgis/rest/services/ROADNET/njdt/MapServer'
      })
    })
   ...
   */
  addTileLayer  () {
    let url_temp = 'http://114.215.146.210:8883/tile?lid=traffic_jx&get=map&cache=off&x={x}&y={y}&z={z}'
    let timeFlag = '1498631144477'
    let templayer = new ol.layer.Tile({
      source: new ol.source.XYZ({
        tileUrlFunction: function (tileCoord) {
          debugger
          var url = url_temp.replace('{z}', (tileCoord[0]).toString())
            .replace('{x}', tileCoord[1].toString())
            .replace('{y}', (-tileCoord[2] - 1).toString());
  ...
  getTempVectorLayer (layerName, params) {
    ...
    if (this.map && vectorLayer) {
      let _layer = this.getLayerByName(layerName)
  ...
  addPolyline (feature, params) {
    ...
    if(feature instanceof Array){
      geom = new ol.geom.LineString(feature)
    } else if (typeof feature == 'string') {
      geom = this.wktToGeom(feature)
    }
    var linesFeat = new ol.Feature({
      geometry: geom
    });
    linesFeat.setStyle(new ol.style.Style({
      fill: new ol.style.Fill({
        color: 'green'
      }),
      stroke: new ol.style.Stroke({
    ...
    let extent = linesFeat.getGeometry().getExtent();
    this.zoomToExtent(extent, false);
    let layer = this.getTempVectorLayer(params['layerName'], {create: true})
    layer.getSource().addFeature(linesFeat);
  };

  addPolylineMet (feature, params) {
    let layer = this.getTempVectorLayer(params['layerName'], {create: true})
    let [geom, coor] = [null]
    if(feature instanceof Array){
      coor = feature
      ...
      if(geom.getType() == 'MultiLineString'){
        coor = geom.getCoordinates()[0]
      }else{
        coor = geom.getCoordinates()
      }
    }
    let [coordinates, cosVal] = [null]
    let self = this
    let [i, coorLen] = [0, coor.length];
    setInterval(function (){
      let source = layer.getSource()
      let f = source.getFeatureById('pointFeat')
      if(f){
        source.removeFeature(f)
      }
      let [arr1, arr2] = [coor[i], null]
      if(coordinates == null){
        arr2 = coor[i + 1]
      } else {
        arr2 = coordinates
      }
      let [x1, x2, y1, y2] = [arr1[0], arr2[0], arr1[1], arr2[1]]
      let a = (y2 - y1)/(x2 - x1)
      let b = y2 - a * x2
      let temp1 = Math.pow((x2 - x1), 2)
      let temp2 = Math.pow((y2 - y1), 2)
      let temp = temp1 + temp2
      cosVal = Math.acos((x2 - x1)/(Math.sqrt(temp)))
      // 计算两点间的距离是否小于100
      let len = Math.sqrt(temp)
      if (len > 2) {
        let x = x1 + Math.sqrt(2) * (x2 - x1) / Math.sqrt(temp)
        let y = y1 + Math.sqrt(2) * (y2 - y1) / Math.sqrt(temp)
        coordinates = [x, y]
      } else {
        coordinates = coor[2]
      }
      let sss = (coordinates[0] + b / a) / (Math.sqrt(Math.pow((coordinates[0] + b / a), 2) + Math.pow(coordinates[1], 2)))
      let angle = Math.acos(sss)
      if(a < 0){
        if(y1 < y2){
      ...
      self.addP(coordinates, params, layer, angle)
      if(i == coorLen - 2){
        i = 0
      } else {
        i++
  ...
  addP (coordinates, params, layer, cosVal) {
    ...
    feat.setStyle(new ol.style.Style({
      ...
      image: new ol.style.Icon({
        ...
        rotation: (Math.PI * cosVal)/180
      })
    }))
    feat.setId('pointFeat')
  ...
  /**
   * 地图上添加点
   * @param feature支持两种格式（1.geometry 如：[12127870.35928337, 3755068.0562904645]       2.wkt  如：POINT (12127870.35928337 3755068.0562904645)）
   * @param params 格式：{
          layerName: 'vectorLabel'
        }
   */
  addPoints1 (feature, params) {
    let geom = null
    if (feature instanceof Array) {
      geom = new ol.geom.Point(feature)
    } else if (typeof feature === 'string') {
      let wkt = new ol.format.WKT()
      geom = wkt.readGeometry(feature)
    }
    let feat = new ol.Feature({
      geometry: geom
    })
  ...
  splitPoint (feature, params) {
    ...
    this.addPoints(coordinates, params)
  }

  addPoints (coors, flag) {
    let layer  = this.getTempVectorLayer('testLayer', {create: true})
    let [i, len] = [0, coors[0].length];
    setInterval(function (){
      let source = layer.getSource()
      let f = source.getFeatureById('pointFeat')
      if(f){
      ...
      let feat = new ol.Feature({
        geometry: new ol.geom.Point(coors[0][i])
      })
      feat.setStyle(new ol.style.Style({
        fill: new ol.style.Fill({
          color: '#000'
  ...
  addPointsOverlay (feature, dom) {
    debugger
    var popup = new ol.Overlay({
      element: dom,
      positioning: 'center-center',
      position: [12039538.837394118, 3822521.218460439],
      offset: [0, 0]
    });
   ...
   * @param params
   */
  addPolylines (features) {
    let geom = new ol.format.WKT().readGeometry(features)
    let feat = new ol.Feature({
      geometry: new ol.geom.MultiLineString(geom[0])
    });
    feat.setStyle(new ol.style.Style({
      fill: new ol.style.Fill({
        color: 'red'
    ...
    debugger
    let layer = this.getTempVectorLayer('testline', {create: true})
    layer.getSource().addFeature(feat)
  };
